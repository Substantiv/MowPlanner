...
class Local_Planner():
    def __init__(self):
        ...
        self.__pub_rtc_cmd = rospy.Publisher('/willand/drive_controller/cmd_vel', Twist, queue_size=10)             # cmd_vel的发布者，发布由MPC求解出的最优控制
        self._model_states = rospy.Subscriber('/willand/gazebo_odom', Odometry, self._model_states_cb)              # gazebo 模型状态的订阅者，订阅里程计获得gazebo模型的位置
        self._sub_traj_waypts = rospy.Subscriber('/kino_astar/path', Path, self._vomp_path_callback)                  # 期望轨迹的订阅者，订阅规划期发布的期望路点
    ...
    def _get_yaw_from_quaternion(self, quat):
        """
        从四元数中提取出偏航角（yaw）
        :param quat: 四元数，geometry_msgs/Quaternion类型
        :return: 偏航角 yaw
        """
        # 使用四元数转换为偏航角
        orientation = quat
        ...
        yaw = np.arctan2(siny_cosp, cosy_cosp)  # 返回[-pi, pi]范围内的yaw
        return yaw
