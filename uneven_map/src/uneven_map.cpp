...
namespace uneven_planner
{
    ...
    void UnevenMap::init(ros::NodeHandle& nh)
    {
        ...
        // 使用 pcl::CropBox 对点云进行裁剪，提取一个特定区域内的点云
        ...
        // 使用 pcl::VoxelGrid 对点云进行体素滤波，减小点云的分辨率，从而加速后续处理
        ...
        // occ map
        // 根据条件更新占用地图：如果某个体素的常数值低于 min_cnormal 或者其 sigma 值大于 max_rho，则将该体素标记为占用。
